...
class PlaneData(DataModule):
    def __init__(self, data_frame, trunc_num = 5000, batch_size=64, 
        ...
        all_flights = pd.concat(normal_flights)
        all_flights = all_flights.drop(["time", "flight_id", "label", "is_fail"], axis=1)
        ...
        for flight in normal_flights:
            ...
            flight = flight.drop(["time", "flight_id", "label", "is_fail"], axis=1)
...
class Trainer(d2l.Trainer):
    ...
    def fit(self, model, data):
        ...
        for self.epoch in range(self.max_epochs):
            self.fit_epoch()
            self.scheduler.step()
            plt.savefig("./losses2_StandardScaler.png")
...
model = MainModel(rnn, num_features, num_hiddens, dropout, lr)
...
# model.load_state_dict(torch.load('./models/LSTM_all_256_0.2_split_10_45_45.pth', weights_only=True,  map_location=torch.device('cpu')))
# trainer = Trainer(max_epochs=100, num_gpus=8)
...
# tmp = tmp.drop(["time", "flight_id", "label", "is_fail"], axis=1)
...
# plt.savefig("./tmp_StandardScaler.pnmodel_weights_without_labels_MSE.pthg")
